...
#pragma endregion
	private: System::Void btnRegister_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		{
			MessageBox::Show("Hicbir kisim bos girilemez");
		}
		else
		{
			String^ hashed_password = sha1hash::sha1(tboxPassword->Text);

			// MyConnection içindeki bağlantı kontrolünü çalıştırdım. Kapalıysa bağlantıyı açtı.
			MyConnection::CheckConnection();

			// SqlCommand hazır fonksiyon, yazdığınız yazıyı çalıştırılabilir bir querye dönüştürür. Değerleri querye parametre olarak gönderdik.
			...
			// Queryde yazdığımız parametreleri, burada doldurduk.
